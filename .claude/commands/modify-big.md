---
name: modify-big
description: Make a substantial or cross-cutting change to existing CoupleGoAI code using Claude's native multiagent workflow with strong shared context and explicit handoffs.
---
...
Use Claude's own best available multiagent techniques to deliver a large existing-code change without any external orchestration dependency.

The main Claude session is the integrator. Subagents support it; they do not replace it.
...
1. `CLAUDE.md`
2. `.claude/skills/supabase.md`
3. `.claude/skills/react-native.md`
4. `.claude/skills/agent-workflow.md`
...
Use only the native subagents in `.claude/agents/`:
...
5. Keep the main Claude session responsible for conflict resolution, integration, and final verification.
...
All rules from `CLAUDE.md` and the skill files apply without exception.
