...
This skill governs how ALL agents in this project behave using Claude's native workflow only.
...
| `/modify-big` | Large or cross-cutting existing-feature change. Use Claude-native multiagent workflow with structured handoffs and explicit file ownership. |
...
When using subagents, the main Claude session is the integrator. It must gather context first, then pass a shared brief to every subagent instead of relying on implicit memory.
...
- Main Claude performs the final integration, verification, and user-facing report
